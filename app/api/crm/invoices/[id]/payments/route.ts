...
export async function POST(request: Request, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!payment.replayed) {
      try {
        await notifyOrganizationRoles({ organizationId: session.user.organizationId, roles: ['ceo', 'manager', 'finance'], excludeUserId: session.user.id, eventType: 'invoice.payment_received', subject: `Payment received for invoice ${payment.invoice.number}`, eyebrow: 'Finance notification', title: 'Invoice payment received', summary: `${session.user.fullName} recorded a payment against an invoice.`, fields: [{ label: 'Invoice', value: payment.invoice.number }, { label: 'Payment', value: String(body.amount) }, { label: 'Status', value: payment.invoice.status }, { label: 'Reference', value: body.reference || 'Not provided' }], action: { label: 'Open Finance & HR', url: `${process.env.APP_URL || 'https://ipaytechops.com'}/operations?module=Finance%20%26%20HR` } });
