...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [organization, settings, locations, roles] = await Promise.all([
      ...
      query('SELECT timezone, currency, date_format, updated_at FROM organization_settings WHERE organization_id = $1', [session.user.organizationId]),
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const updated = await withTransaction(async client => {
      ...
      await client.query(`INSERT INTO organization_settings (organization_id, timezone, currency, date_format, updated_by)
        VALUES ($1, $2, $3, $4, $5)
        ON CONFLICT (organization_id) DO UPDATE SET timezone = EXCLUDED.timezone, currency = EXCLUDED.currency, date_format = EXCLUDED.date_format, updated_by = EXCLUDED.updated_by, updated_at = now()`, [session.user.organizationId, body.timezone, body.currency, body.dateFormat, session.user.id]);
