...
export async function POST(request: Request, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    void notifyOrganizationRoles({ organizationId: session.user.organizationId, roles: ['ceo', 'manager'], excludeUserId: session.user.id, eventType: 'warranty.claim_resolved', subject: `Warranty claim ${result.rows[0].number} ${result.rows[0].status.toLowerCase()}`, eyebrow: 'Warranty oversight', title: 'Warranty claim resolved', summary: `${session.user.fullName} resolved a warranty claim.`, fields: [{ label: 'Claim', value: result.rows[0].number }, { label: 'Outcome', value: result.rows[0].status }, { label: 'Resolution', value: result.rows[0].resolution }], action: { label: 'Open warranty', url: `${process.env.APP_URL || 'https://ipaytechops.com'}/operations?module=Warranty` } });
