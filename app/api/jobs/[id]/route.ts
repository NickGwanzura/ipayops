...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (body.installerId) { const installer = await query(`SELECT id FROM users WHERE id = $1 AND organization_id = $2 AND is_active = true AND role IN ('installer', 'operator', 'manager', 'admin', 'ceo')`, [body.installerId, auth.session.user.organizationId]); if (!installer.rows[0]) return NextResponse.json({ error: 'Installer is not an active member of this organization.' }, { status: 400 }); }
