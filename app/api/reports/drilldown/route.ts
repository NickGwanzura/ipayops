...
export async function GET(request: NextRequest) {
  ...
  const values = type === 'stock' ? [session.user.organizationId, region, product] : type === 'pipeline' ? [session.user.organizationId, from, to] : [session.user.organizationId, from, to, region, product];
