...
export default function RoleDashboard({ role, settings, query, onNavigate }: Props) {
  ...
  if (loading) return <section className="panel role-loading" aria-label="Loading role dashboard"><div className="role-loading-skeleton"><span/><span/><span/><span/></div><div className="role-loading-skeleton wide"/></section>;
...
function FinanceDashboard({ data, settings, query, onNavigate, error }: Omit<Props, 'role'> & { data: Record<string, Payload>; error: string }) {
  ...
  return <RoleFrame eyebrow="Finance desk" title="Finance dashboard" subtitle="Payments, debtors, invoices, expenses, and commission settlement" role="finance" onNavigate={onNavigate} error={error}>
    ...
    <div className="grid-main"><RolePanel title="Debtors and payments" subtitle={`Showing ${Math.min(invoices.length, 6)} of ${invoices.length} invoices`} action={{ label: 'Open finance', onClick: () => onNavigate('Finance & HR') }}><RoleList items={invoices.filter(invoice => !query || `${invoice.number} ${invoice.client_name || ''} ${invoice.status}`.toLowerCase().includes(query.toLowerCase())).slice(0, 6).map(invoice => ({ title: invoice.number, detail: `${invoice.client_name || 'Client'} · ${formatCurrency(invoice.outstanding, settings.currency)} · ${invoice.status}`, module: 'Finance & HR' as OpsModule }))} empty="No outstanding invoices." onNavigate={onNavigate}/></RolePanel><RolePanel title="Expense queue" subtitle={`Showing ${Math.min(expenses.length, 6)} of ${expenses.length} expenses`} action={{ label: 'View all expenses', onClick: () => onNavigate('Finance & HR') }}><RoleList items={expenses.slice(0, 6).map(expense => ({ title: expense.number, detail: `${expense.description} · ${formatCurrency(expense.amount, settings.currency)} · ${expense.status}`, module: 'Finance & HR' as OpsModule }))} empty="No expenses recorded." onNavigate={onNavigate}/></RolePanel></div>
  </RoleFrame>;
}
...
function RolePanel({ title, subtitle, action, children }: { title: string; subtitle: string; action?: { label: string; onClick: () => void }; children: React.ReactNode }) { return <section className="panel"><div className="panel-header"><div><h2>{title}</h2><p>{subtitle}</p></div>{action && <button className="text-btn" onClick={action.onClick}>{action.label} <ArrowUpRight size={14}/></button>}</div>{children}</section>; }
