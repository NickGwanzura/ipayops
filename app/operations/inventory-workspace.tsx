...
export default function InventoryWorkspace({ query, notify }: { query: string; notify: (message: string) => void }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setSummary(summaryData.summary || {}); setItems(inventoryData.inventory || []); setTransfers(transferData.transfers || []); setShipments(shipmentData.shipments || []); setReservations(reservationData.reservations || []);
  ...
  return <>
    ...
    <Panel title="Transfers, shipping, and reservations" subtitle="Dispatch available serials, reserve exact units, receive transfers, and track shipments"><div className="transfer-form"><label><span>Source</span><select value={source} onChange={e => setSource(e.target.value)}><option value="">Select source</option>{locations.map(location => <option key={location}>{location}</option>)}</select></label><label><span>Destination</span><input value={destination} onChange={e => setDestination(e.target.value)} placeholder="Bulawayo Branch"/></label><label><span>Reservation reference</span><input value={reservationRef} onChange={e => setReservationRef(e.target.value)} placeholder="QUO-2026-000001"/></label><button className="ops-btn blue" onClick={() => void createTransfer()}>Dispatch selected</button></div><div className="data-table"><TableHead labels={['Select','Serial','SKU','Location','Status','Action']}/>{availableItems.slice(0, 20).map(item => <div className="data-row transfer-row" key={item.id}><input type="checkbox" checked={selected.includes(item.id)} onChange={e => setSelected(current => e.target.checked ? [...current, item.id] : current.filter(id => id !== item.id))}/><strong>{item.serial_number}</strong><span>{item.sku}</span><span>{item.location}</span><Status value={item.status}/><button className="row-action" onClick={() => void reserve(item)}><ClipboardCheck size={14}/> Reserve</button></div>)}</div>{transfers.length > 0 && <div className="transfer-list">{transfers.slice(0, 8).map(transfer => <div className="transfer-card" key={transfer.id}><div><strong>{transfer.number}</strong><span>{transfer.source_location} → {transfer.destination_location} · {transfer.items.length} serials</span></div><Status value={transfer.status}/><div className="transfer-card-actions">{transfer.status === 'In transit' && <button className="row-action" onClick={() => void receiveTransfer(transfer)}><PackageCheck size={14}/> Receive</button>}{transfer.status === 'In transit' && <><input value={carrier} onChange={e => setCarrier(e.target.value)} placeholder="Carrier"/><input value={tracking} onChange={e => setTracking(e.target.value)} placeholder="Tracking no."/><button className="row-action" onClick={() => void createShipment(transfer)}><Truck size={14}/> Ship</button></>}</div></div>)}</div>}{shipments.length > 0 && <div className="transfer-list"><strong className="workflow-help">Shipment status</strong>{shipments.slice(0, 8).map(shipment => <div className="transfer-card" key={shipment.id}><div><strong>{shipment.number}</strong><span>{shipment.carrier || 'No carrier'} · {shipment.tracking_number || 'No tracking number'}</span></div><select value={shipment.status} onChange={e => void updateShipment(shipment, e.target.value)}><option>Draft</option><option>Dispatched</option><option>In transit</option><option>Delivered</option><option>Cancelled</option></select></div>)}</div>}{reservations.length > 0 && <div className="transfer-list"><strong className="workflow-help">Active reservations</strong>{reservations.slice(0, 8).map(reservation => <div className="transfer-card" key={reservation.id}><div><strong>{reservation.serial_number}</strong><span>{reservation.reference_type} · {reservation.reference_id}</span></div><button className="row-action" onClick={() => void releaseReservation(reservation)}>Release</button></div>)}</div>}{error && <p className="workflow-error" role="alert">{error}</p>}</Panel>
  </>;
}
